<div class="app-sidebar sidebar-shadow">
    <div class="app-header__logo">
        <div class="logo-src"></div>
        <div class="header__pane ml-auto">
            <div>
                <button type="button" class="hamburger close-sidebar-btn hamburger--elastic" data-class="closed-sidebar">
                    <span class="hamburger-box">
                        <span class="hamburger-inner"></span>
                    </span>
                </button>
            </div>
        </div>
    </div>
    <div class="app-header__mobile-menu">
        <div>
            <button type="button" class="hamburger hamburger--elastic mobile-toggle-nav">
                <span class="hamburger-box">
                    <span class="hamburger-inner"></span>
                </span>
            </button>
        </div>
    </div>
    <div class="app-header__menu">
        <span>
            <button type="button" class="btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav">
                <span class="btn-icon-wrapper">
                    <i class="fa fa-ellipsis-v fa-w-6"></i>
                </span>
            </button>
        </span>
    </div>    <div class="scrollbar-sidebar">
        <div class="app-sidebar__inner">
                <h6 class="widget-heading" style="fond-weight:bolder; margin-bottom:30px; color:black; text-align:center;">Collège François Xavier Vogt
                    <span style="font-size:0.9em; color:grey;">Année Scolaire 2021-2022</span>
                </h6>
            <ul class="vertical-nav-menu">
                <li class="app-sidebar__heading">Tableau de Bord</li>
                <li>
                    <a href="index.html" class="mm-active">
                        <i class="metismenu-icon pe-7s-graph1"></i>
                        Tableau de bord
                    </a>
                </li>
                <li class="app-sidebar__heading">Ressources</li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-study"></i>
                        Apprenants
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_apprenant")}}">
                                <i class="metismenu-icon"></i>
                                Liste
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.creer_apprenant")}}">
                                <i class="metismenu-icon">
                                </i>Créer un Apprenant
                            </a>
                        </li>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-add-user"></i>
                        Enseignants
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_enseignant")}}">
                                <i class="metismenu-icon">
                                </i>Liste
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.creer_enseignant")}}">
                                <i class="metismenu-icon">
                                </i>Créer un Enseignant
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon">
                                </i>Emploi du temps
                            </a>
                        </li>

                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-folder"></i>
                        Classes
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_classe")}}">
                                <i class="metismenu-icon">
                                </i>Liste de classes
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_classe")}}">
                                <i class="metismenu-icon">
                                </i>Créer une Classe
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon">
                                </i>Emploi du temps
                            </a>
                        </li>

                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-note2"></i>
                        Matières
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_matiere")}}">
                                <i class="metismenu-icon">
                                </i>Liste des matières
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.creer_matiere")}}">
                                <i class="metismenu-icon">
                                </i>Créer une Matière
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon">
                                </i>Groupe de Matières
                            </a>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-help2"></i>
                        Cycles
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_cycle")}}">
                                <i class="metismenu-icon">
                                </i>Liste
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_cycle")}}">
                                <i class="metismenu-icon">
                                </i>Créer un Cycle
                            </a>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-network"></i>
                        Niveaux Scolaires
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_niveau")}}">
                                <i class="metismenu-icon">
                                </i>Liste
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_niveau")}}">
                                <i class="metismenu-icon">
                                </i>Créer un Niveau
                            </a>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-next"></i>
                        Périodes
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_periode")}}">
                                <i class="metismenu-icon">
                                </i>Liste des Périodes
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_periode")}}">
                                <i class="metismenu-icon">
                                </i>Créer une Période
                            </a>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-albums"></i>
                        Sous-Périodes
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_sous_periode")}}">
                                <i class="metismenu-icon">
                                </i>Liste des sous-Périodes
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_sous_periode")}}">
                                <i class="metismenu-icon">
                                </i>Créer une sous-Période
                            </a>
                        </li>
                    </ul>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-news-paper"></i>
                        Evaluations
                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                    </a>
                    <ul>
                        <li>
                            <a href="{{route("gestscol.list_evaluation")}}">
                                <i class="metismenu-icon">
                                </i>Liste des evaluations
                            </a>
                        </li>
                        <li>
                            <a href="{{route("gestscol.formulaire_evaluation")}}">
                                <i class="metismenu-icon">
                                </i>Créer un type d'evaluation
                            </a>
                        </li>
                    </ul>
                </li>
                <li  >
                    <a href="{{route("gestscol.matricule_etablissement")}}">
                        <i class="metismenu-icon pe-7s-display2"></i>
                        Matricules
                    </a>
                </li>


                <li class="app-sidebar__heading">Configurations</li>
                <li>
                    <a href="{{route("gestscol.classe_apprenant")}}">
                        <i class="metismenu-icon pe-7s-display2"></i>
                        Elèves dans les classes
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-display2"></i>
                        Elèves et matières d'option
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-users"></i>
                        Elèves composants les groupes
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-shuffle"></i>
                        Affectations des matières
                    </a>
                </li>
                <li>
                    <a href="{{route("gestscol.parametrage_matiere")}}">
                        <i class="metismenu-icon pe-7s-share"></i>
                        Paramétrages des matières
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-way"></i>
                        Gestion des moyennes
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-like2"></i>
                        Appréciations de notes
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-star"></i>
                        Mention du travail
                    </a>
                </li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-medal"></i>
                        bourses
                    </a>
                </li>
                <li class="app-sidebar__heading">Discipline</li>
                <li>
                    <a href="#">
                        <i class="metismenu-icon pe-7s-mouse">
                        </i>Consultation
                    </a>
                </li>
                <li>
                    <a href="{{route("gestscol.liste_sanction")}}">
                        <i class="metismenu-icon pe-7s-eyedropper">
                        </i>Sanction
                    </a>
                </li>
                <li>
                    <a href="{{route("gestscol.sanction.parametrage")}}">
                        <i class="metismenu-icon pe-7s-pendrive">
                        </i>Paramétrage de Sanctions
                    </a>
                </li>
            </ul>
        </div>
    </div>
</div>
